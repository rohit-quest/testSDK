  ...
  const onClick = (option: number) => {
    setCheckedState((prevState) => {
      if (prevState.includes(options[option])) {
        return prevState.filter((item) => item !== options[option]);
      } else {
        return [...prevState, options[option]];
      }
    }
    );
  }
  ...
  return (
    <div className="q_multi_choice_box">
      {options.map((option: string, id: number) => (
        <div style={style} className="q_mult_choice_option" key={id} onClick={() => {onClick(id); handleChange(option)}}>
          {checkedState?.includes(option) ?
            (<CheckBoxImg backgroundColor={checkedColor} />)
