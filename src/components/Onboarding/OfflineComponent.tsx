    ...
    function returnAnswers() {
        ...
        for (let i of Object.keys(crt)) {
            if (i.includes("/manual") && crt[i] != "") {
                ...
                let criteriaDetails: offlineFormData[] = offlineFormData.filter((item) => item.criteriaId == id)
            ...
            <div className="q-onb-home" style={{
                ...
                >
                    ...
                    <div className="q-onb-main-first">
                        {(template === "multi-question") && (offlineFormData.length > 0) && (designState.length > 1) && (!!progress?.length) && (<ProgressBar />)}
                        {!!designState && designState.length > 0 && checkDesignCriteria()
                    ...
                    {(offlineFormData && showFooter) && <QuestLabs style={styleConfig?.Footer} />}
                </div>
            </div>
        )
    )
