...
export default function DailyStreak({
    ...
    const { apiKey, entityId, apiType, apiSecret, themeConfig } = useContext(QuestContext.Context);
    ...
    const descriptionStyle = {color: themeConfig.secondaryColor, ...styleConfig?.Description || {}};

    return (
        <div className="q_daily_streak" style={{background: themeConfig.backgroundColor,color: themeConfig.secondaryColor, ...styleConfig?.Form}}>
            <div className="q_steak_days_box">
                <div className="q_streak_days">{days}</div>
                <div className="q_steak_days_text"  style={descriptionStyle}>Streak days</div>
            </div>
            <div className="q_streak_desc" style={descriptionStyle}>{stepDetails[currentActive].description}</div>
                    ...
                    stepDetails.map((step, i) => {
                        ...
                        return (
                            <div key={i}
                                ...
                                <img src={imgSrc} alt="" />
                                <div className={"q_streak_step_title " + (isActive ? "q_streak_step_active" : "q_streak_step_disable")}>{step.title}</div>
